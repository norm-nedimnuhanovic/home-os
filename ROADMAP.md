...
- [ ] Configure a Supabase Storage bucket for `Document.fileRef` (Life Admin) and `Transaction.attachment` (Finance) — the mechanism is built and verified (`scripts/setup-storage-bucket.ts` / `pnpm run storage:setup`, `docs/upload.md` §3.3, `README.md` step 4), confirmed working against local dev Supabase Storage during Life Admin's browser testing; still unchecked because it hasn't been run against the now-real project yet
- [ ] Create the Resend account, verify the sending domain, wire `RESEND_API_KEY`/`EMAIL_FROM` as Vercel env vars — invite emails/notifications will fail cleanly (no crash, matches the designed rollback/error-surfacing behavior, §8) until this is done
- [x] Link the Vercel project (`vercel link`) — `nedimnuhanovic-imelbas-projects/home-os` (`vercel.com/nedimnuhanovic-imelbas-projects/home-os`). GitHub auto-connect failed at link time (`400`: the Vercel account needs a GitHub Login Connection added first, at vercel.com/account — a manual, account-level step); repo isn't yet wired for auto-deploy-on-push, first deploy was triggered manually via `vercel --prod` instead.
