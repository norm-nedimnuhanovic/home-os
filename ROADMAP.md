...
- [x] Create the Resend account, wire `RESEND_API_KEY`/`EMAIL_FROM` as Vercel Production+Preview env vars — verified with a real send via the Resend API (`POST /emails`, real `id` returned, delivery confirmed). **Sending domain not yet verified** — `EMAIL_FROM` is temporarily `Home OS <onboarding@resend.dev>`, Resend's own shared testing address, which rejects any `to` outside the account owner's own inbox with a `422 validation_error` (confirmed directly against the API) rather than silently dropping it — `sendHouseholdInviteEmail()`'s existing try/catch surfaces this the same clear way as an unconfigured key does (§8), so inviting anyone else in the household fails loudly, not silently. Once a real domain is bought and verified on Resend's Domains tab, only `EMAIL_FROM` needs to change — no code, no other env var.
