...
Jobs" PR, 2026-07-26. This file is the single source of truth for "what's
...
- [x] `vercel.json` — the five Cron entries from `docs/email.md` §9.6
- [x] Local dev database: Docker Postgres 16 container (`home-os-postgres`, `docker run -d --name home-os-postgres -e POSTGRES_USER=homeos -e POSTGRES_PASSWORD=homeos_dev_password -e POSTGRES_DB=homeos -p 5432:5432 postgres:16-alpine`), `.env`'s `DATABASE_URL`/`DIRECT_URL` point at it. **Swap both for real Supabase pooled/direct connection strings before deploying** — this container is dev-only.
...
- [ ] Provision the real Supabase project; replace the local-Postgres `DATABASE_URL`/`DIRECT_URL` above with its pooled/direct connection strings
- [ ] Wire Supabase Auth as the platform Auth capability referenced by `Member.emailVerifiedAt` (session/login mechanics — out of scope for `plan.md` itself, but required before any module can authenticate a request)
- [ ] Configure a Supabase Storage bucket for `Document.fileRef` (Life Admin) and `Transaction.attachment` (Finance) — the mechanism is built and verified (`scripts/setup-storage-bucket.ts` / `pnpm run storage:setup`, `docs/upload.md` §3.3, `README.md` step 4), confirmed working against local dev Supabase Storage during Life Admin's browser testing; still unchecked because it hasn't been run against a *real* provisioned Supabase project, same as the other "provision the real X" lines here
- [ ] Create the Resend account, verify the sending domain, wire `RESEND_API_KEY`
- [ ] Link the Vercel project (`vercel link`)
- [ ] Wire CI (lint + `tsc --noEmit` + test) to run on every PR
...
- [x] In-app bell/inbox reads `Notification` for every category **not** already backed by `Reminder`/`ReminderOccurrence` (`src/lib/notifications/dispatch.ts`'s `fanOutNotificationsForOccurrence` — wires `task.assigned`/`share.received`/`household.invite_received`; the email half now actually sends, wrapped in try/catch so a Resend failure never breaks the triggering action — see §8)
...
### Entities
- [x] `Task` — householdId, title, description, dueDate, dueDateAllDay, priority (`low`/`medium`/`high`/`urgent`), assigneeId (single — no multi-assignee), createdById, completedAt + completedById, parentTaskId (one level of sub-task nesting only), seriesId + recurrenceRuleId, boardId/columnId/boardPosition (optional), visibility, sourceModule/sourceEntityId
...
`reopenTask` actions, `getTask`/`getVisibleTasks`/`getHouseholdTags`/
