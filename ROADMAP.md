...
- [x] **Password reset — real backend, no UI at all until now** (user-requested, found while looking for genuine gaps: `requestPasswordReset()`/`updatePassword()` were both fully written, correctly following security best-practice — never reveals whether an email has an account — and `docs/auth.md` §8 already specified the exact intended file paths, but `app/(auth)/reset-password/request/page.tsx` and `.../update/page.tsx` were never built, and `/login` had no "Forgot password?" link at all — completely unreachable, same "built with no caller" pattern as several other gaps this session). Closed in this pass: both pages built matching `/login`'s/`/signup`'s own established look (plain `useState`-driven form, no react-hook-form — those two pages never used it either) and error-handling convention (call the action directly, no try/catch, since `<Input required>`'s native HTML5 validation is what actually guards against the zod-parse-throws-on-invalid-input case in practice, matching `/login`'s own accepted shape); a "Forgot password?" link added next to `/login`'s Password label. **Also closed in the same pass**: both actions had zero test coverage before this (confirmed by grep) — 4 new cases added to `(auth)/actions.test.ts` (happy path + rejected path for each). Deliberately does **not** add a toast on the update-password success path — it `redirect()`s server-side inside the action itself (same as `login()`), so no client-side code after the call ever runs on the success path to fire one; a toast here would be genuinely dead code, not just unnecessary. Browser-verified: the request page's "check your email"-style message renders and stays visible (a persistent inline message, not a toast, deliberately — same reasoning as `/signup`'s own existing message, which needs to still be readable after the transient-notification window a toast would give it), and the update page renders correctly. **Real, much bigger production bug found the moment the user actually tested the full email round-trip (not just the UI)**: both the "Confirm signup" and "Reset password" Supabase Auth email templates were still using Supabase's own stock `{{ .ConfirmationURL }}` — `docs/auth.md` §2.3 point 4 always specified they should point at our own `/api/auth/callback` with `token_hash`/`type`/`next`, but this was apparently never actually applied to the real project, and had gone unnoticed until this session because the earlier `site_url` fix (§8) happened to make Supabase's own stock confirmation flow work well enough that nobody had reason to look closer. Confirmed directly via the Management API (`GET /v1/projects/{ref}/config/auth`) rather than guessing — both `mailer_templates_*_content` fields were still the stock HTML. Attempting to `PATCH` them directly failed with a real, informative error: `"Email template modification is not available for free tier projects using the default email provider."` — turns out custom SMTP (§2.3 point 2, also never actually configured — `smtp_host`/`smtp_user`/`smtp_pass` were all `null`) is a **hard prerequisite** for template edits on the free tier, not just a nice-to-have as the doc's ordering implied. Fixed in order: user configured custom SMTP via Resend directly in the Supabase dashboard (Host `smtp.resend.com`, Port `465`, Username `resend`, the same `RESEND_API_KEY` the app itself already uses, Sender `notifications@nedimnuhanovic.com`), confirmed via the Management API that `smtp_host` etc. were no longer null, then the same template `PATCH` that failed before succeeded (200) with the exact HTML `docs/auth.md` specifies. User-confirmed end-to-end on production: requested a real reset email, clicked the link, it worked.
- [x] **Real production data-integrity bug: duplicate signup for an already-registered email silently created a second `Household` + `Member` instead of erroring** (user asked, out of curiosity, "does the app validate signup with an email that already has an account?" — tested locally first and it correctly showed "User already registered", but the user then tested on **production** and reported no message appeared on the form at all). Root cause, confirmed via the Supabase Management API's `database/query` endpoint rather than guessed: Supabase's own anti-email-enumeration behavior for `supabase.auth.signUp()` does **not** reliably error for an already-registered email on this project — it silently creates a brand-new `auth.users` row with a new id and returns an ambiguous "success" instead. This reproduced even for the user's real, actively-used, **confirmed** account (not just an unconfirmed leftover, which was the first, wrong hypothesis) — a direct SQL query turned up **three** separate `Member`/`Household` rows for the same email, each from a separate signup attempt, each with a different `supabaseUserId`. Not reproducible against the separate local Supabase Docker instance, which correctly rejects the duplicate both times — production-only behavior. Fixed at the application level in `signUpAndCreateHousehold()` (`src/app/(auth)/actions.ts`): a `prismaAuthBootstrap.member.findFirst({ where: { email } })` pre-check now runs **before** `createServerSupabaseClient()`/`supabase.auth.signUp()` is ever called, short-circuiting with `"An account already exists for this email address. Try logging in instead."` — since Supabase itself can't be trusted to catch this, the app now does. 2 new tests added to `(auth)/actions.test.ts` (existing member found → error, `signUp` never called; no existing member → proceeds normally). Browser-verified locally. The 2 stray duplicate `Household`/`Member` rows already created in production were cleaned up directly (user approved with "da") once the code fix shipped.
...
- [x] In-app bell/inbox reads `Notification` for every category **not** already backed by `Reminder`/`ReminderOccurrence` (`src/lib/notifications/dispatch.ts`'s `fanOutNotificationsForOccurrence` — wires `task.assigned`/`share.received`/`household.invite_received`; the email half now actually sends, wrapped in try/catch so a Resend failure never breaks the triggering action — see §8). **The "bell/inbox" half of this line was aspirational until now, not actually built**: `getInbox()`/`markNotificationRead()` existed with zero callers anywhere in the app (confirmed by grep before starting) — toggling a category's "In-app" preference silently wrote `Notification` rows nobody could ever see. Closed in this pass: `src/components/app-shell/notification-bell.tsx` (Server Component, mirrors `Nav`'s own shape — fetches `getInbox(member)`, hands off to a Client Component) + `notification-bell-button.tsx` (a `Popover`-based bell, unread-count badge, click-to-mark-read, "Mark all read"), wired into `(app)/layout.tsx` alongside Quick Capture/Search. New `markAllNotificationsRead()` action (`src/lib/notifications/actions/mark-all-read.ts`) — `markNotificationRead()` only ever handled one at a time. Both `getInbox()` and `markNotificationRead()` also had zero test coverage before this pass (the same "built with no caller, so no test either" pattern as other harness gaps this session) — `get-inbox.test.ts`/`mark-read.test.ts`/`mark-all-read.test.ts` added. **Real, unrelated bug found while browser-verifying this against the local seed household**: assigning a task to a *local seed* member (`sam@seed.local` etc.) always fails silently — `task-form.tsx`'s `assigneeId` selection shows correctly in the UI but the form never submits, because `entities/task.ts`'s `createTaskInputSchema` validates `assigneeId: z.string().cuid()`, and `prisma/seed/constants.ts`'s `SEED_MEMBER_OWNER_ID`/`SEED_MEMBER_ADMIN_ID`/`SEED_MEMBER_MEMBER_ID` are human-readable strings (`"seed-member-sam"` etc.), not real CUIDs — every local seed member fails `.cuid()` outright. **Not fixed here** — real production households (Supabase-generated `@default(cuid())` ids) are unaffected, so this is a local-dev-only gap, and fixing it means picking a side (relax the schema's `.cuid()` to a plain `.string()`, or reissue the seed with real cuid-shaped ids) that's out of this change's scope; the bell/notification flow itself (badge count, popover list, mark-read) is verified locally against the empty-inbox state; the full task-assignment → notification path is verified against production's real household (two real member accounts, real cuid ids): assigning a task from one account produced an immediate unread badge for the other, the popover showed "Task assigned" with a relative timestamp, and clicking it cleared the badge — confirmed via the second account's own session, not just the assigner's. **Follow-up, same day (user-requested after seeing the bell's actual output)**: the notification text itself was still just the event type's own generic label ("Task assigned", "Item shared with you") — no indication of *which* task or *who* did it, since `Notification.sourceEntityType`/`sourceEntityId`/`body` were columns that existed in the schema but were never populated by `fanOutNotificationsForOccurrence()`. Fixed by adding a small per-category `buildNotificationDetail()` in `dispatch.ts`: `task.assigned` now looks up the `Task.title` and the assigner's `Member.displayName` (one query each, via `Promise.all`) to produce a body like `Priya assigned you "Water the plants"`; `share.received` looks up the sharer's name to produce `Sam shared a Note with you`. Both fall back to the old generic-label-only behavior if the source row no longer resolves (deleted since), rather than erroring the whole fan-out. `notification-bell-button.tsx` updated to actually render `item.body` (previously fetched via `getInbox()` but never displayed) and to navigate on click for categories with a known list route (`SOURCE_ENTITY_HREF`, currently just `Task → /tasks` — Tasks has no per-item detail route, so this only gets a member to the right list, never the exact row). `household.invite_received` intentionally left at the generic fallback — it's registered but never actually emitted anywhere (the invite email bypasses this pipeline entirely), so building detail-lookup for it would be dead code. **Critical bug shipped and caught minutes later via a real browser test on production**: the first version of `buildNotificationDetail()`'s three new `prisma.task.findUnique`/`prisma.member.findUnique` calls all used `where: { id }` alone — `Task` and `Member` are both tenant-guarded models (`src/lib/db/tenant-guard.ts`), so every one of them threw `Refusing *.findUnique: missing householdId` the instant a real `task.assigned` fired with `inAppEnabled` on. Because `fanOutNotificationsForOccurrence()` is awaited synchronously inside `createTask()`'s own call chain (`createTask` → `emitTaskAssigned` → `emitEvent` → `dispatchToSubscribers` → here), the throw propagated all the way up and broke **task assignment itself** — visible in the browser as "An error occurred in the Server Components render," dialog stuck open, even though the `Task` row had already committed before the throw (so the task silently existed despite the client seeing a hard failure). The unit tests written alongside the original change didn't catch it — they asserted the exact `where: { id }` shape the buggy code itself produced, mocks don't enforce the real tenant-guard's behavior, so a wrong assertion and wrong implementation matched each other perfectly. Caught only by actually creating a real task assignment in a browser against the live deployment, the same detection gap this ROADMAP has flagged repeatedly for other modules (`dispatchToSubscribers`, `removeMember`/`changeMemberRole`/`transferOwnership`, `ModuleGrant`'s compound key). Fixed by adding `householdId: occurrence.householdId` to all three `where` clauses and correcting the test assertions to require it — a straightforward instance of CLAUDE.md rule 1 ("both, always — not just id"), missed here despite having written that exact rule elsewhere in this same session.
...
- [x] `NoteLink` renders as a chip/card **on both sides** (user-requested "quick win," originally only the note's own side was built) — `NoteDetail`'s "Linked to" section (resolving the linked Task/Event's title, not a raw id — a real bug caught via an actual browser test and fixed, at the time this line was written) was the only direction; now Task's Edit dialog (`task-row-actions.tsx`) and Event's detail dialog (`event-detail-dialog.tsx`) both render a "Linked notes" section pointing back the other way. No new query needed — `NoteLink` already carries polymorphic-target convenience relations (`linkedTaskId`/`linkedTask`, `linkedEventId`/`linkedEvent`, docs/orm-conventions.md §4) that were sitting unused; `getVisibleTasks()`/`getCalendarRange()` just needed `include: { noteLinks: { include: { note: { select: { id, title } } } } }` added to their existing queries, one shared query each (no N+1 per row). Each linked note renders as a `Link` to `/notes/[noteId]` in the palette's `--primary` color, falling back to "Untitled note" for a note with no title (`Note.title` is optional). Browser-verified: linked a note to both a task and the seed "Family dinner" event via the existing `NoteLinkDialog`, confirmed both the task's Edit dialog and the event's detail dialog show the note title as a working link.
- [x] **`NoteLink` extended to `Subscription`** (user-requested follow-up — "sad kad je Finance modul gotov" — `subscription` was already a valid `noteLinkedEntityTypeSchema` enum value and `linkNote()` already had a `linkedSubscriptionId` branch, both written back when Finance didn't exist yet and left unreachable since `NoteLinkDialog`'s own type-select only ever offered Task/Event). Closed both directions in the same pass, matching Task/Event's shape exactly: `NoteLinkDialog` gained a `Subscription` option (+ a `LINKED_ENTITY_MODULE` lookup replacing the old inline `type === "task" ? "tasks" : "calendar"` ternary, now that there are three, not two) and a `subscriptions` prop; `NoteDetail`'s `resolveLinkLabel()` gained the `subscription` branch; `notes/[noteId]/page.tsx` now also fetches `getSubscriptions(member.householdId)` (all statuses, unfiltered — matching how the existing linkable-tasks/events lists aren't filtered by completed/past either) for the linkable list. Reverse side: `getSubscriptions()` gained the same `noteLinks` include as `getVisibleTasks()`/`getCalendarRange()`, and `subscription-row-actions.tsx`'s Edit dialog renders the same "Linked notes" section as Task's. Browser-verified: linked a note to the seed "Internet service" subscription, confirmed the subscription's own Edit dialog shows it.
...
- [x] **Color palette overhaul + sidebar UX fixes** (user feedback: the stock shadcn "neutral" scaffold — every token in `globals.css` was `oklch(... 0 0)`, literally zero chroma anywhere — read as sterile/generic, "ova osnovna bijela boja mi nije sjela"). `globals.css`'s `:root`/`.dark` blocks rewritten around one accent hue (a muted sage green, ~155° in OKLCH) applied to `--primary`/`--ring`/`--sidebar-primary`/`--sidebar-accent`, plus a warm off-white background/foreground (~75-85° hue, very low chroma) replacing pure white/black — `--destructive` and Finance's income/expense red/green left untouched since those carry semantic meaning. Dark-mode tokens updated in parallel for palette coherence, though unreachable at the time (`next-themes`' `useTheme()` was called by `sonner.tsx` but no `<ThemeProvider>` was mounted anywhere and there was no toggle) — **since fixed, see the dark mode toggle entry below.** Two follow-up fixes in the same pass: **(1)** `src/components/ui/button.tsx`'s shared `buttonVariants` never set `cursor-pointer` (Tailwind/shadcn's base button resets to `cursor: default`, matching the native browser default, not `pointer`) — every button in the app showed an arrow cursor on hover; fixed once, at the shared component, plus `disabled:cursor-not-allowed` alongside the pre-existing `disabled:opacity-50`. **This claim didn't hold up** — it only ever covered the shared `Button` component; a follow-up pass (user-noticed, on the login/signup "switch to the other auth page" links specifically) found 13 more raw `<button>` elements across the app (tag-toggle chips in `task-form.tsx`/`note-form.tsx`/`transaction-form.tsx`/`visibility-field.tsx`/`split-editor.tsx`, row-opening buttons in `contact-list.tsx`/`document-list.tsx`/`category-list.tsx`, `calendar-item-chip.tsx`, the two auth-page switch links, and the notification bell's two internal buttons) that never went through the shared component at all, so the earlier fix never reached them — each got its own explicit `cursor-pointer` added. **(2)** No UI anywhere showed *who* was logged in — new `src/components/app-shell/user-menu.tsx` (colorTag-tinted initial avatar + displayName + role, title-attribute tooltip) rendered next to "Log out", pinned to the bottom of the desktop sidebar (mobile keeps it inline in the top icon strip instead — no vertical room for a real "bottom" there, so the same `userFooter` JSX fragment in `layout.tsx` is rendered twice, toggled by breakpoint rather than fighting one element's flex-direction across both). **Real bug found while placing it at the bottom**: pinning it via `mt-auto` alone did nothing useful on a long page (e.g. `/settings/modules`) — `<nav>` had no independent height or position, so it scrolled away *with* the page instead of staying put, meaning "bottom of the sidebar" was only reachable by scrolling the entire page down first. Fixed by making the desktop sidebar `sticky top-0 h-screen overflow-y-auto`, so it's pinned to the viewport and scrolls independently of `<main>`. Verified via `pnpm typecheck`/`lint`/`test` (460 passing)/`build`, all clean, plus direct browser screenshots (desktop + mobile) confirming the palette, cursor, avatar/name/role placement, and independent sidebar scroll all render correctly.
- [x] **Dark mode toggle** (user-requested "quick win," since the tokens above already existed and just needed switching on) — `src/components/theme-provider.tsx` (thin `next-themes` wrapper) mounted in the root `src/app/layout.tsx` (`attribute="class"`, `defaultTheme="system"`, `enableSystem`; `<html>` gets `suppressHydrationWarning` since `next-themes` sets the `.dark` class client-side, before React can reconcile it), plus a new `src/components/app-shell/theme-toggle.tsx` button (Sun/Moon icon, wired into `(app)/layout.tsx`'s utility row alongside Notifications/Quick Capture/Search). Renders a disabled placeholder until mounted client-side — `next-themes` can't know the resolved theme during SSR (reads `localStorage`/`matchMedia`), so rendering the real icon any earlier risks a flash of the wrong icon and a hydration mismatch. Browser-verified (this was also the first real look at dark mode anywhere in the app, since it was never reachable before): toggled dark on `/dashboard`, `/tasks`, `/finance` — palette reads correctly (warm dark charcoal, sage accent, readable contrast throughout), toggled back to light, and confirmed the choice persists across a full page reload (`next-themes`' own `localStorage` persistence, no app code needed for that part).
- [x] **Custom favicon** (user-requested "quick win") — the app was still shipping `create-next-app`'s default `favicon.ico`. `src/app/icon.svg` (Next.js App Router's file-convention icon — automatically injects `<link rel="icon" href="/icon.svg" type="image/svg+xml" sizes="any">` alongside the existing `favicon.ico` link, no metadata code needed): a simple rounded-square house glyph in the palette's own `--primary` sage green (`#225a39` — read out via an actual `canvas.fillStyle` resolve of `oklch(0.42 0.08 155)` rather than eyeballing a hex, to guarantee it's the exact same green as the rest of the UI). Modern browsers prefer the scalable SVG over the old `.ico`; the `.ico` stays as-is for the rare renderer that doesn't support SVG favicons. Verified: `pnpm build` emits the `/icon.svg` route, the rendered `<head>` carries both `<link>` tags, and a direct screenshot of `/icon.svg` confirms the glyph renders correctly.
...
- [x] **"Back to X" link on every parent-less sub-page** (user-requested — "ima na nekim mjestima gdje se covjek hoce vratiti na osnovni modul") — one new shared `src/components/back-link.tsx` (a plain server-renderable `Link` + `ChevronLeft`, no state, safe inside either a Server Component page or a `"use client"` detail component), dropped into the 9 pages that had no way back to their own module's hub besides re-clicking the sidebar: `/kanban/[boardId]`, `/notes/[noteId]`, `/life-admin/shopping-lists/[listId]` (all three genuinely parent-less — no other in-page nav existed), plus `/life-admin/documents`, `/life-admin/renewals`, `/life-admin/contacts`, `/finance/budgets`, `/finance/subscriptions`, `/finance/settlements` (the user's own follow-up catch: these looked like they had sibling-tab navigation from a distance, but each one is actually a dead end on its own — the Documents/Renewals/Contacts and Budgets/Subscriptions/Settlements "tabs" only exist on the Life Admin/Finance *hub* pages themselves, not repeated on each sub-page). Verified: `pnpm typecheck`/`lint`/`test` (470 passing)/`build` all clean; browser-verified the link renders correctly (scoped past a same-labeled sidebar nav link ambiguity in the first verification attempt — `main`-scoped locator, not an app bug) and that clicking it actually navigates to the right hub page.
