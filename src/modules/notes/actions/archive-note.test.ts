...
describe("archiveNote", () => {
  ...
  it("archives the note when the acting member is its author (happy path)", async () => {
  ...
  it("rejects when the acting member isn't the note's author (rejected path)", async () => {
    ...
    await expect(archiveNote("note_1")).rejects.toThrow("Only the note's author can archive it.");
