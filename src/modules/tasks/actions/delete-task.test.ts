...
describe("deleteTask", () => {
  ...
  it("archives the task when the acting member created it (happy path)", async () => {
    ...
    vi.mocked(prisma.task.update).mockResolvedValue({ id: "task_1", householdId: "household_1" } as never);
    ...
    expect(cancelTaskDueReminder).toHaveBeenCalledWith({ id: "task_1", householdId: "household_1" });
  });
  ...
  it("rejects when the acting member neither created nor is assigned to the task (rejected path)", async () => {
    ...
    await expect(deleteTask("task_1")).rejects.toThrow(
